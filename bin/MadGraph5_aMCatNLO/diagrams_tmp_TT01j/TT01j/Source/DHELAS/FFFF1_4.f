...
C     
     ...
     $ * W4))
      F4(3)= DENOM*CI * TMP95*F1(3)*M4
      F4(4)= DENOM*CI * TMP95*F1(4)*M4
      F4(5)= DENOM*(-CI )* TMP95*(F1(3)*(-1D0)*(P4(0)+P4(3))+F1(4)*(
     $ +CI*(P4(2))-P4(1)))
...
C     ProjM(2,3)*ProjM(4,1)
C     
      SUBROUTINE FFFF1_10_4(F1, F2, F3, COUP1, COUP2, M4, W4,F4)
      IMPLICIT NONE
      COMPLEX*16 CI
      PARAMETER (CI=(0D0,1D0))
      COMPLEX*16 DENOM
      ...
      CALL FFFF10_4(F1,F2,F3,COUP2,M4,W4,FTMP)
      DO I = 3, 6
        F4(I) = F4(I) + FTMP(I)
      ENDDO
      END
